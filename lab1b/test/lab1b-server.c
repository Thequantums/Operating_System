...
void parentprocess() {
	...
	while(1) {
		...
		int i =0;
		bytecomp = 256 - input_shell.avail_out ;
		...
		memset(c_buff,0,bytecomp); //reset c_buff
	}
	// no compress flag
	else {
		int in = 0;
	...
	memset(buffer, 0, byteread); //reset buffer.
	...
	if (pfds[1].revents & POLLIN) {
		...
		int byte_comp = 256 - output_shell.avail_out;
		index = 0;
		//write(newsockfd, &cc_buff, byte_comp);	
