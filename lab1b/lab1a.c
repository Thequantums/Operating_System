...
int log_mode = 0;
int compress_mode = 0;
char* logfd; // file descriptor for log file
int sockfd;
...
void parentprocess(int socketfd) {
	...
	while(1) {
		...
		//decompress data
		if (compress_flag) {
			char c_buff[256]; //creating buffer for decompression
		...
		char c; // used to read one character at a time
		int i =0;
		bytecomp = byteread - to_server.avail_out ;
	...
	if (pfds[1].revents & POLLIN) {
		...
		index = 0;
			while (index < br) {
				...
				//writting each log as a byte;
				if (c4 == cr || c4 ==lf) {
				write(logfd, "RECEIVED a byte: ", 17);
				write(logfd, &crlf, 2);
				write(logfd, "\n", 1);
				}
				else {
		...
		if (compress_mode) {
			char c1_buff[256]; //creating buffer for decompression
		...
		int bytecomp1 = br - from_server.avail_out ;
...
if (log_mode) {
	logfd = open(logfile, O_RDWR | O_CREAT | O_APPEND, 0666);
}
